...
/*   Updated: 2025/04/08 12:38:35 by sniemela         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_rays(t_data *data, float ray_angle, mlx_image_t *ray)
{
	...
	while (i <= 66)
	{
		xy = get_index_of_rov_and_col(data, (int)(data->player.x), (int)(data->player.y));
		map_y = data->player.y;
		map_x = data->player.x;
		while (data->map[xy] == FLOOR)
		{
			...
			xy = get_index_of_rov_and_col(data, (int)map_x, (int)map_y);
			mlx_put_pixel(data->ray, map_x * data->tile_mini - 6, map_y * data->tile_mini - 6, RED);
		}
